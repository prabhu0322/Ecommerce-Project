...
<body>
    ...
    <script>
            ...
            function load_data(){
                $.ajax({
                    'type' : "GET",
                    'url' : "routes.php?route=ajax-load-main-page-data",
                    'data' : {},
                    'beforeSend': function() {
                        document.getElementById('product-list-container').innerHTML = `<div class="col-md-12 text-center ">Loading Products...</div>`;
                    },
                    'success' : function( d ){
                        //console.log(d);
                       let items = JSON.parse( d );
                                    ...
                                    <div class="item-card">
                                        <div class="card" style="width: 18rem;">
                                            <a data-fancybox="gallery"
                                                href="uploads/${item['image']}">
                                                <img src="uploads/${item['image']}" class="card-img-top" alt="...">
                                            </a>
                                            <div class="card-body">
                                                <h5 title="${item['product_name']}" class="card-title">${item['product_name']}</h5>
                                                <p title="${item['product_description']}" class="card-text">${item['product_description'].slice(0,30)+"..."}</p>
                                                <p class="card-text">${item['price']}</p>
                                                <a href="routes.php?route=product-show&product_id=${item['product_id']}" class="btn btn-primary">view details</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        document.getElementById('product-list-container').innerHTML = content;
                    },
                });
            }
    </script>
